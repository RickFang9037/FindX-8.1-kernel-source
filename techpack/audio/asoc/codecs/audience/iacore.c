...
void iacore_enable_esd_irq(struct iacore_priv *iacore)
{
	mutex_lock(&iacore->esd_irq_lock);
	pr_info("enter\n");
	if (!iacore->esd_irq_enabled) {
		enable_irq(iacore->pdata->esd_irq_pin);
		iacore->esd_irq_enabled = true;
		pr_info("done\n");
	}
	mutex_unlock(&iacore->esd_irq_lock);
}

void iacore_disable_esd_irq(struct iacore_priv *iacore)
{
	mutex_lock(&iacore->esd_irq_lock);
	pr_info("enter\n");
	if (iacore->esd_irq_enabled) {
		disable_irq(iacore->pdata->esd_irq_pin);
		iacore->esd_irq_enabled = false;
		pr_info("done\n");
	}
	mutex_unlock(&iacore->esd_irq_lock);
}

void iacore_disable_esd_irq_nosync(struct iacore_priv *iacore)
{
	mutex_lock(&iacore->esd_irq_lock);
	pr_debug("enter\n");
	if (iacore->esd_irq_enabled) {
		disable_irq_nosync(iacore->pdata->esd_irq_pin);
		iacore->esd_irq_enabled = false;
		pr_debug("done\n");
	}
	pr_debug("leave\n");
