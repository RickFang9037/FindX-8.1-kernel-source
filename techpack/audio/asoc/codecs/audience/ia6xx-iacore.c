...
#ifdef VENDOR_EDIT
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/06/29, Add for smartmic ESD issue
#ifdef CONFIG_SND_SOC_IA_ESD
extern int dmic1_highz_enable(u8 enable);
#endif /* CONFIG_SND_SOC_IA_ESD */
#endif /* VENDOR_EDIT */
...
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/06/29, Add for smartmic ESD issue
#ifdef CONFIG_SND_SOC_IA_ESD
		iacore_disable_esd_irq_nosync(iacore);
#endif /* CONFIG_SND_SOC_IA_ESD */
#endif /* VENDOR_EDIT */
...
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/06/29, Add for smartmic ESD issue
#ifdef CONFIG_SND_SOC_IA_ESD
irqreturn_t iacore_esd_reset_work(int irq, void *data) {
	struct iacore_priv *iacore = (struct iacore_priv *)data;
	pr_info("iacore_esd_reset_work!!!\n");

	if (iacore_get_power_state(iacore) == SPL_PDM_RECORD) {
            pr_info("pdm record state\n");
            dmic1_highz_enable(1);
            msleep(50);
            dmic1_highz_enable(0);
	}
        msleep(60*1000);
        return IRQ_HANDLED;
}
...
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/06/29, Add for smartmic ESD issue
#ifdef CONFIG_SND_SOC_IA_ESD
	pdata->esd_irq_pin = of_get_named_gpio(dev->of_node,
			"adnc,reset", 0);
	if (pdata->esd_irq_pin < 0) {
		dev_err(dev, "%s(): get adnc,reset failed\n", __func__);
		pdata->esd_irq_pin = -1;
	}
	pr_info("gpio for pdata->esd_irq_pin %d\n",pdata->esd_irq_pin);
#endif /* CONFIG_SND_SOC_IA_ESD */
#endif /* VENDOR_EDIT */
...
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/06/29, Add for smartmic ESD issue
#ifdef CONFIG_SND_SOC_IA_ESD
	mutex_init(&iacore->esd_irq_lock);
#endif /* CONFIG_SND_SOC_IA_ESD */
#endif /* VENDOR_EDIT */
...
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/06/29, Add for smartmic ESD issue
#ifdef CONFIG_SND_SOC_IA_ESD
	if (pdata->esd_irq_pin != -1) {
		rc = gpio_request(pdata->esd_irq_pin, "ia6xx_esd_rst");
		if (rc < 0) {
			pr_err("%s(): ia6xx_esd_rst gpio request fail %d\n",
				__func__, rc);
			goto exit;
		}

		rc = gpio_direction_input(pdata->esd_irq_pin);
		gpio_set_value(pdata->esd_irq_pin, 1);
		pr_err("%s(): pdata->esd_irq_pin %d,  %d: \n", __func__, pdata->esd_irq_pin, gpio_get_value(pdata->esd_irq_pin));
		if (rc < 0) {
			pr_err("%s(): ia6xx_esd_rst gpio set input fail %d\n",
				__func__, rc);
			goto exit;
		}

	} else {
		pr_err("%s(): ia6xx_esd_rst undefined\n",
			 __func__);
	}
#endif /* CONFIG_SND_SOC_IA_ESD */
#endif /* VENDOR_EDIT */
