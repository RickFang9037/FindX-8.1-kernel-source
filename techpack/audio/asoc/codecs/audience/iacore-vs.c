...
//Zhaoan.Xu@PSW.MM.AudioDriver.Codec, 2018/06/29, Add for smartmic ESD issue
	...
	if (iacore->pdata->esd_irq_pin != -1) {
		int irq_pin;
		pr_info("gpio_get_value(iacore->pdata->esd_irq_pin;: %d!\n", gpio_get_value(iacore->pdata->esd_irq_pin));
		irq_pin = gpio_to_irq(iacore->pdata->esd_irq_pin);
		iacore->pdata->esd_irq_pin = irq_pin;
		irq_set_status_flags(irq_pin, IRQ_DISABLE_UNLAZY);
		rc = request_threaded_irq(irq_pin,
					NULL,
					iacore_esd_reset_work,
					IRQF_TRIGGER_LOW | IRQF_ONESHOT| IRQF_EARLY_RESUME | IRQF_NO_SUSPEND,
					"iacore_esd_reset_work", iacore);
		if (rc) {
			pr_err("event esd request_threaded_irq()  fail %d\n", rc);
			goto err_exit;
		}
		...
		gpio_set_value(iacore->pdata->esd_irq_pin, 1);
		pr_info("gpio_get_value(iacore->pdata->esd_irq_pin  %d!\n", gpio_get_value(iacore->pdata->esd_irq_pin));
	} else {
		pr_info("ia6xx esd irq pin undefined \n");
	}
